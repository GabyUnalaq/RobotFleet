...
It does not implement obstacle avoidance, but rather focuses on
flocking behavior.
"""
...
class DWAFlockingController(GenericController):
    FUTURE_STEPS = 30
    V_SAMPLES = 10
    W_SAMPLES = 17
    def __init__(self):
    ...
    def generate_ctrl(self):
        self.last_ctrl = self.ctrl if len(self.ctrl) else [(0.0, 0.0)] * len(self.robots)
        ...
        for i, bot in enumerate(self.robots):
            ...
            prev_v, prev_w = self.last_ctrl[i]
            v_samples = np.linspace(0.0, MAX_LINEAR_SPEED, DWAFlockingController.V_SAMPLES)
            w_samples = np.random.normal(loc=0.0, scale=MAX_ANGULAR_TURN / 2, size=DWAFlockingController.W_SAMPLES)
            ...
            for v in v_samples:
                for w in w_samples:
                    ...
                    for _ in range(DWAFlockingController.FUTURE_STEPS):
                        x += v * np.cos(theta) * dt
                        y += v * np.sin(theta) * dt
                        theta += w * dt
                    ...
                    velocity_score = v
                    score = alignment_score + 0.1 * velocity_score + smoothness_penalty
                    if score > best_score:
                        best_score = score
                        best_ctrl = (v, w)

            # Smooth output control to avoid abrupt changes
            smoothed_v = 0.7 * self.last_ctrl[i][0] + 0.3 * best_ctrl[0]
            smoothed_w = 0.7 * self.last_ctrl[i][1] + 0.3 * best_ctrl[1]
            self.ctrl[i] = (smoothed_v, smoothed_w)
